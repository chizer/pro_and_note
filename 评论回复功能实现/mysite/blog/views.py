...
def get_blog_list_common_data(request,blogs_all_list):
    ...
    current_page_range = [x for x in range(int(current_page_num)-2,int(current_page_num)+3) if 0 < x <= paginator.num_pages]    #根据当前页码确定显示的上下2页
...
def blog_detail(request,blog_pk):
    ...
    context['comments'] = comments.order_by('-comment_time')  #返回本blog对应的评论
    #实例化form表单 并添加默认属性值  blog_content_type.model 为contentype的一个字段 str类型 并返回
